export const dataBerita = [
    {
        id: 1,
        image: "https://via.placeholder.com/600x400?text=Santri+di+Pesantren",
        title: "Pesantren Al-Hikmah Gelar Lomba Tilawah Al-Quran",
        content: "Pondok Pesantren Al-Hikmah menggelar lomba tilawah Al-Quran sebagai bagian dari kegiatan tahunan mereka lorem ipsum...Pondok Pesantren Baiturrahman menyelenggarakan diskusi agama yang bertema...",
        author: "Ustadz Ahmad",
        date: "2024-09-15"
    },
    {
        id: 2,
        image: "https://via.placeholder.com/600x400?text=Diskusi+Agama",
        title: "Diskusi Agama: Mendalami Makna Hadis dan Aplikasinya dalam Kehidupan Sehari-hari",
        content: "Pondok Pesantren Baiturrahman menyelenggarakan diskusi agama yang bertema...",
        author: "Ustadzah Fatimah",
        date: "2024-09-16"
    },
    {
        id: 3,
        image: "https://via.placeholder.com/600x400?text=Festival+Seni+dan+Budaya",
        title: "Festival Seni dan Budaya: Merayakan Keanekaragaman Warisan Budaya di Pondok Pesantren Raudhatul Hasanah",
        content: "Pondok Pesantren Raudhatul Hasanah menggelar Festival Seni dan Budaya...",
        author: "Ustadz Muhammad",
        date: "2024-09-17"
    },
    {
        id: 4,
        image: "https://via.placeholder.com/600x400?text=Tahfidz+Al-Quran",
        title: "Tahfidz Al-Quran: Meningkatkan Kualitas Hafalan Santri di Pesantren Bina Ummah",
        content: "Pesantren Bina Ummah mengadakan program Tahfidz Al-Quran...",
        author: "Ustadz Ali",
        date: "2024-09-18"
    },
    {
        id: 5,
        image: "https://via.placeholder.com/600x400?text=Kegiatan+Ekstrakurikuler",
        title: "Kegiatan Ekstrakurikuler: Mengembangkan Bakat dan Minat Santri di Pesantren Madinatul Ilmi",
        content: "Pesantren Madinatul Ilmi tidak hanya fokus pada pendidikan agama...",
        author: "Ustadzah Zainab",
        date: "2024-09-19"
    },
    {
        id: 6,
        image: "https://via.placeholder.com/600x400?text=Kajian+Kitab+Kuning",
        title: "Kajian Kitab Kuning: Mendalami Ilmu Keislaman di Pesantren Darul Ulum",
        content: "Pesantren Darul Ulum rutin mengadakan kajian kitab kuning...",
        author: "Ustadz Hasan",
        date: "2024-09-20"
    },
    {
        id: 7,
        image: "https://via.placeholder.com/600x400?text=Kegiatan+Pembinaan+Akhlak",
        title: "Kegiatan Pembinaan Akhlak: Membentuk Karakter Santri di Pesantren Hidayatullah",
        content: "Pesantren Hidayatullah memiliki program pembinaan akhlak...",
        author: "Ustadzah Aisyah",
        date: "2024-09-21"
    },
    {
        id: 8,
        image: "https://via.placeholder.com/600x400?text=Kegiatan+Pembelajaran",
        title: "Kegiatan Pembelajaran: Menyajikan Metode Pendidikan Terbaik di Pesantren Al-Falah",
        content: "Pesantren Al-Falah mengusung konsep pembelajaran yang menekankan pada penerapan metode pendidikan terbaik...",
        author: "Ustadzah Maryam",
        date: "2024-09-22"
    },
    {
        id: 9,
        image: "https://via.placeholder.com/600x400?text=Kegiatan+Keagamaan",
        title: "Kegiatan Keagamaan: Meningkatkan Ketaatan Beragama Santri di Pesantren An-Nur",
        content: "Pesantren An-Nur mengadakan berbagai kegiatan keagamaan...",
        author: "Ustadz Yusuf",
        date: "2024-09-23"
    },
    {
        id: 10,
        image: "https://via.placeholder.com/600x400?text=Kegiatan+Kemasyarakatan",
        title: "Kegiatan Kemasyarakatan: Mendorong Santri untuk Berkontribusi pada Masyarakat di Pesantren Modern",
        content: "Pesantren Modern mengajarkan pentingnya berkontribusi pada masyarakat melalui berbagai kegiatan...",
        author: "Ustadz Abdullah",
        date: "2024-09-24"
    }
];

export const artikelData = [
    {
        id: 1,
        image: "https://via.placeholder.com/600x400?text=Artikel+1",
        title: "Kegiatan Pembelajaran Tahfidz Al-Quran di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien memiliki program pembelajaran Tahfidz Al-Quran yang intensif. Para santri diajarkan untuk menghafal Al-Quran dengan tartil dan tajwid yang baik. Program ini berlangsung setiap hari dan diawasi oleh para ustadz yang berpengalaman dalam bidang tahfidz.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 2,
        image: "https://via.placeholder.com/600x400?text=Artikel+2",
        title: "Diskusi Agama: Mendalami Ilmu Keislaman di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien sering mengadakan diskusi agama yang bertujuan untuk memperdalam pemahaman santri terhadap ajaran Islam. Diskusi-diskusi ini dipandu oleh kyai terkemuka dan membahas berbagai topik dalam ilmu keislaman.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 3,
        image: "https://via.placeholder.com/600x400?text=Artikel+3",
        title: "Kegiatan Seni Budaya: Menciptakan Kreativitas di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien tidak hanya fokus pada pendidikan agama, tetapi juga memberikan perhatian pada pengembangan seni dan budaya. Setiap tahun, pesantren ini menggelar festival seni dan budaya yang menampilkan bakat-bakat seni santri dalam bidang tari, musik, dan seni rupa.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 4,
        image: "https://via.placeholder.com/600x400?text=Artikel+4",
        title: "Kegiatan Keagamaan Rutin di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien mengadakan kegiatan keagamaan rutin seperti shalat berjamaah, pengajian kitab kuning, dan kelas-kelas pengajian. Kegiatan ini bertujuan untuk memperkuat keimanan dan ketakwaan santri serta memperdalam pemahaman mereka terhadap ajaran Islam.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 5,
        image: "https://via.placeholder.com/600x400?text=Artikel+5",
        title: "Kegiatan Pendidikan Karakter di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Selain aspek keagamaan, Pondok Pesantren Hidayatul Mubtadi'ien juga menekankan pentingnya pendidikan karakter. Melalui berbagai kegiatan seperti ceramah, tazkiyatun nafs, dan bimbingan rohani, pesantren berupaya membentuk santri yang berakhlak mulia dan mampu menjadi pemimpin yang amanah.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 6,
        image: "https://via.placeholder.com/600x400?text=Artikel+6",
        title: "Kegiatan Ekstrakurikuler di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien menyediakan berbagai kegiatan ekstrakurikuler untuk mengembangkan bakat dan minat santri. Mulai dari olahraga, seni, hingga kewirausahaan, semua dirancang untuk memberikan kesempatan kepada santri untuk berkembang sesuai dengan potensi mereka.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 7,
        image: "https://via.placeholder.com/600x400?text=Artikel+7",
        title: "Kegiatan Kemasyarakatan di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien mendorong santri untuk berkontribusi pada masyarakat melalui berbagai kegiatan kemasyarakatan. Ini termasuk pengabdian kepada masyarakat, program pemberdayaan ekonomi, dan kegiatan sosial lainnya yang bertujuan untuk membentuk santri yang peduli dan bertanggung jawab.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 8,
        image: "https://via.placeholder.com/600x400?text=Artikel+8",
        title: "Pengembangan Literasi di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien juga memberikan perhatian pada pengembangan literasi di antara santri. Melalui program-program seperti perpustakaan, diskusi buku, dan kegiatan menulis, pesantren berupaya meningkatkan minat dan keterampilan literasi santri.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 9,
        image: "https://via.placeholder.com/600x400?text=Artikel+9",
        title: "Pendidikan Kewirausahaan di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Selain pendidikan agama, Pondok Pesantren Hidayatul Mubtadi'ien juga memberikan pelatihan kewirausahaan kepada santri. Tujuannya adalah untuk membekali mereka dengan keterampilan dan pengetahuan yang diperlukan untuk menjadi pengusaha yang sukses di masa depan.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    },
    {
        id: 10,
        image: "https://via.placeholder.com/600x400?text=Artikel+10",
        title: "Peran Teknologi dalam Pendidikan di Pondok Pesantren Hidayatul Mubtadi'ien",
        content: "Pondok Pesantren Hidayatul Mubtadi'ien tidak ketinggalan dalam memanfaatkan teknologi dalam pendidikan. Mereka menggunakan teknologi untuk meningkatkan kualitas pembelajaran, memberikan akses ke sumber daya pendidikan yang lebih luas, dan mempersiapkan santri untuk menghadapi tantangan masa depan.",
        date: "2024-09-13",
        author: "Kholik Muzaki"
    }
];

export const albums = [
    {
        id: 1,
        title: 'Album 1',
        description: 'Keterangan singkat tentang album 1.',
        images: [
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+1',
                caption: 'Gambar 1'
            },
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+2',
                caption: 'Gambar 2'
            },
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+3',
                caption: 'Gambar 3'
            },
        ],
    },
    {
        id: 2,
        title: 'Album 2',
        description: 'Keterangan singkat tentang album 2.',
        images: [
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+4',
                caption: 'Gambar 4'
            },
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+5',
                caption: 'Gambar 5'
            },
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+6',
                caption: 'Gambar 6'
            },
        ],
    },
    {
        id: 3,
        title: 'Album 3',
        description: 'Keterangan singkat tentang album 3.',
        images: [
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+7',
                caption: 'Gambar 7'
            },
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+8',
                caption: 'Gambar 8'
            },
            {
                src: 'https://via.placeholder.com/800x400?text=Gambar+9',
                caption: 'Gambar 9'
            },
        ],
    }
];

export const linkVideo = [
    {
        id: 1,
        link: "https://www.youtube.com/embed/SA5sFIPAMMc",
        judul: "Sholawat Bersama menyamut tahun baru hijriyah"
    },
    {
        id: 2,
        link: "https://www.youtube.com/embed/dSG8Gzl0wPo",
        judul: "Sholawat Bersama menyamut tahun baru hijriyah"
    },
    {
        id: 3,
        link: "https://www.youtube.com/embed/9qRLoS_mETI",
        judul: "Sholawat Bersama menyamut tahun baru hijriyah"
    },
    {
        id: 4,
        link: "https://www.youtube.com/embed/D3-gtrCLuUw",
        judul: "Sholawat Bersama menyamut tahun baru hijriyah"
    },
    {
        id: 5,
        link: "https://www.youtube.com/embed/g3wRx9NsphM",
        judul: "Sholawat Bersama menyamut tahun baru hijriyah"
    },
    {
        id: 6,
        link: "https://www.youtube.com/embed/hxiUeqMoiiE",
        judul: "Sholawat Bersama menyamut tahun baru hijriyah"
    }
];

export const DataKeuangan = [
    { id: 1, jenisTransaksi: 'Pengeluaran', jumlah: 1000000, tanggal: '2023-09-13', keterangan: 'Pembayaran bulan September 2023' },
    { id: 2, jenisTransaksi: 'Pemasukan', jumlah: 1000000, tanggal: '2023-09-14', keterangan: 'Pemasukan bulan September 2023' },
    { id: 3, jenisTransaksi: 'Pengeluaran', jumlah: 500000, tanggal: '2023-09-15', keterangan: 'Pembelian alat tulis' },
    { id: 4, jenisTransaksi: 'Pemasukan', jumlah: 2500000, tanggal: '2023-09-16', keterangan: 'Sumbangan dari donatur' },
    { id: 5, jenisTransaksi: 'Pengeluaran', jumlah: 1500000, tanggal: '2023-09-17', keterangan: 'Biaya operasional bulan September' },
    { id: 6, jenisTransaksi: 'Pemasukan', jumlah: 3000000, tanggal: '2023-09-18', keterangan: 'Pemasukan dari acara bazar' },
    { id: 7, jenisTransaksi: 'Pengeluaran', jumlah: 1200000, tanggal: '2023-09-19', keterangan: 'Pembayaran listrik bulan September' },
    { id: 8, jenisTransaksi: 'Pemasukan', jumlah: 1800000, tanggal: '2023-09-20', keterangan: 'Pemasukan bulan September dari sponsor' },
    { id: 9, jenisTransaksi: 'Pengeluaran', jumlah: 700000, tanggal: '2023-09-21', keterangan: 'Pembelian makanan untuk acara' },
    { id: 10, jenisTransaksi: 'Pemasukan', jumlah: 4000000, tanggal: '2023-09-22', keterangan: 'Penjualan barang bekas' },
    { id: 11, jenisTransaksi: 'Pengeluaran', jumlah: 250000, tanggal: '2023-09-23', keterangan: 'Pembelian bahan baku' },
    { id: 12, jenisTransaksi: 'Pemasukan', jumlah: 900000, tanggal: '2023-09-24', keterangan: 'Pemasukan dari hasil kerja sama' },
    { id: 13, jenisTransaksi: 'Pengeluaran', jumlah: 300000, tanggal: '2023-09-25', keterangan: 'Biaya transportasi' },
    { id: 14, jenisTransaksi: 'Pemasukan', jumlah: 1200000, tanggal: '2023-09-26', keterangan: 'Pemasukan dari kegiatan pelatihan' },
    { id: 15, jenisTransaksi: 'Pengeluaran', jumlah: 600000, tanggal: '2023-09-27', keterangan: 'Pembelian software' },
    { id: 16, jenisTransaksi: 'Pemasukan', jumlah: 1500000, tanggal: '2023-09-28', keterangan: 'Sumbangan dari alumni' },
    { id: 17, jenisTransaksi: 'Pengeluaran', jumlah: 1100000, tanggal: '2023-09-29', keterangan: 'Pembayaran sewa gedung' },
    { id: 18, jenisTransaksi: 'Pemasukan', jumlah: 2000000, tanggal: '2023-09-30', keterangan: 'Pemasukan dari penjualan merchandise' },
    { id: 19, jenisTransaksi: 'Pengeluaran', jumlah: 350000, tanggal: '2023-10-01', keterangan: 'Pembelian peralatan kebersihan' },
    { id: 20, jenisTransaksi: 'Pemasukan', jumlah: 1800000, tanggal: '2023-10-02', keterangan: 'Donasi dari masyarakat' },
    { id: 21, jenisTransaksi: 'Pengeluaran', jumlah: 900000, tanggal: '2023-10-03', keterangan: 'Pembayaran gaji staf' },
    { id: 22, jenisTransaksi: 'Pemasukan', jumlah: 2500000, tanggal: '2023-10-04', keterangan: 'Pendapatan dari kursus' },
    { id: 23, jenisTransaksi: 'Pengeluaran', jumlah: 400000, tanggal: '2023-10-05', keterangan: 'Biaya pembersihan' },
    { id: 24, jenisTransaksi: 'Pemasukan', jumlah: 1300000, tanggal: '2023-10-06', keterangan: 'Pemasukan dari acara tahunan' },
    { id: 25, jenisTransaksi: 'Pengeluaran', jumlah: 950000, tanggal: '2023-10-07', keterangan: 'Pembelian alat olahraga' },
    { id: 26, jenisTransaksi: 'Pemasukan', jumlah: 2200000, tanggal: '2023-10-08', keterangan: 'Pendapatan dari donasi online' },
    { id: 27, jenisTransaksi: 'Pengeluaran', jumlah: 800000, tanggal: '2023-10-09', keterangan: 'Biaya transportasi acara' },
    { id: 28, jenisTransaksi: 'Pemasukan', jumlah: 1750000, tanggal: '2023-10-10', keterangan: 'Pemasukan dari kerjasama dengan mitra' },
    { id: 29, jenisTransaksi: 'Pengeluaran', jumlah: 650000, tanggal: '2023-10-11', keterangan: 'Pembelian makanan untuk event' },
    { id: 30, jenisTransaksi: 'Pemasukan', jumlah: 3100000, tanggal: '2023-10-12', keterangan: 'Pendapatan dari tiket acara' },
    { id: 31, jenisTransaksi: 'Pengeluaran', jumlah: 200000, tanggal: '2023-10-13', keterangan: 'Biaya perlengkapan sekolah' },
    { id: 32, jenisTransaksi: 'Pemasukan', jumlah: 1250000, tanggal: '2023-10-14', keterangan: 'Sumbangan dari sponsor' },
    { id: 33, jenisTransaksi: 'Pengeluaran', jumlah: 480000, tanggal: '2023-10-15', keterangan: 'Pembelian kebutuhan dapur' },
    { id: 34, jenisTransaksi: 'Pemasukan', jumlah: 2700000, tanggal: '2023-10-16', keterangan: 'Pendapatan dari jualan' },
    { id: 35, jenisTransaksi: 'Pengeluaran', jumlah: 750000, tanggal: '2023-10-17', keterangan: 'Biaya seminar' },
    { id: 36, jenisTransaksi: 'Pemasukan', jumlah: 3000000, tanggal: '2023-10-18', keterangan: 'Donasi acara amal' },
    { id: 37, jenisTransaksi: 'Pengeluaran', jumlah: 600000, tanggal: '2023-10-19', keterangan: 'Pembayaran air' },
    { id: 38, jenisTransaksi: 'Pemasukan', jumlah: 1500000, tanggal: '2023-10-20', keterangan: 'Sumbangan dari yayasan' },
    { id: 39, jenisTransaksi: 'Pengeluaran', jumlah: 1000000, tanggal: '2023-10-21', keterangan: 'Biaya promosi' },
    { id: 40, jenisTransaksi: 'Pemasukan', jumlah: 4000000, tanggal: '2023-10-22', keterangan: 'Pendapatan dari sponsor acara' },
    { id: 41, jenisTransaksi: 'Pengeluaran', jumlah: 850000, tanggal: '2023-10-23', keterangan: 'Pembelian media cetak' },
    { id: 42, jenisTransaksi: 'Pemasukan', jumlah: 1200000, tanggal: '2023-10-24', keterangan: 'Pemasukan dari event musik' },
    { id: 43, jenisTransaksi: 'Pengeluaran', jumlah: 500000, tanggal: '2023-10-25', keterangan: 'Pembelian alat presentasi' },
    { id: 44, jenisTransaksi: 'Pemasukan', jumlah: 2100000, tanggal: '2023-10-26', keterangan: 'Donasi dari perusahaan' },
    { id: 45, jenisTransaksi: 'Pengeluaran', jumlah: 1300000, tanggal: '2023-10-27', keterangan: 'Biaya perawatan gedung' },
    { id: 46, jenisTransaksi: 'Pemasukan', jumlah: 2700000, tanggal: '2023-10-28', keterangan: 'Pendapatan dari acara bazar' },
    { id: 47, jenisTransaksi: 'Pengeluaran', jumlah: 200000, tanggal: '2023-10-29', keterangan: 'Pembelian perlengkapan alat tulis' },
    { id: 48, jenisTransaksi: 'Pemasukan', jumlah: 1400000, tanggal: '2023-10-30', keterangan: 'Pemasukan dari seminar online' },
    { id: 49, jenisTransaksi: 'Pengeluaran', jumlah: 950000, tanggal: '2023-10-31', keterangan: 'Biaya acara akhir tahun' },
    { id: 50, jenisTransaksi: 'Pemasukan', jumlah: 3000000, tanggal: '2023-11-01', keterangan: 'Pendapatan dari crowdfunding' }
];




